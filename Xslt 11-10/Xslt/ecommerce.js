...
//    });
    ...
    function ProcessAjax(pageurl) {
        ...
        $.ajax({ url: pageurl, data: { isajax: true }, success: function (data) {
            $('body .nth-product-2 .product-list-wrapper').html($(data).find('.product-list-wrapper').html());
			
            $('body .ajaxfilterresponse').html($(data).closest('.ajaxfilterresponse').html());
            $('body .ajaxbrandresponse').html($(data).find('.ajaxbrandresponse').html());
            $('body .productpager').remove();
            $(data).find('.productpager').insertAfter($('.ajaxresponse'));
        }
        });
