let productFilterHtmlAjax;

try {

	if ($('.productFilterMapTo').html().length > $('.productFilterToMap').html().length) {
		productFilterHtmlAjax = $('.productFilterMapTo').html()
	} else {
		productFilterHtmlAjax = $('.productFilterToMap').html()
	}
} catch (error) {
...
$(document).ready(function () {
	try {
		...
		let productFilterWrapper = $('.productFilterToMap');

		if ($(window).width() >= 992) {

			if ($('.productFilterMapTo').html().length > $('.productFilterToMap').html().length) {
				productFilterHtml = $('.productFilterMapTo').html()
			} else {
				productFilterHtml = $('.productFilterToMap').html()
			}
			$('.productFilterMapTo').html(productFilterHtml)
			productFilterWrapper.html('')
			bindFilterSidemenuClick()
			...
			$('.productFilterMapTo').html('')
			bindFilterSidemenuClick()
			productFilterWrapper.html(productFilterHtml)
			bindFilterSidemenuClick()
		}
	} catch (error) {
